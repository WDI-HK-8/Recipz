...
server.views({
  ...
  path: Path.join(__dirname, "templates") // Users/lilialaw/GA/Recipz/templates
})

var plugins = [
  { register: require('./routes/static-pages.js')},
  { register: require('./routes/recipes.js')},
  {
	...
	// Require MongoDB
	{
		...
		options:{
			"url": process.env.MONGOLAB_URI || "mongodb://127.0.0.1:27017/recipz",
			"settings": {
				"db": {
					native_parser: false
				}
